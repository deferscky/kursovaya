    ...
    class Program
    {
        ...
        private async Task Run()
        {
            try
            {
                ...
                // Шаг 1: Подключение к серверу (фиксированный порт 5200)
                SetupConnection();

                // Шаг 2: Регистрация или вход
                await HandleAuthentication();

                // Если авторизация успешна - показываем меню
        ...
        // 1. Настройка подключения (фиксированный порт 5200)
        private void SetupConnection()
        {
            try
            {
                // Фиксируем порт 5200 как в методичке
        ...
        // 2. Обработка аутентификации (упрощенная)
        ...
        // ===== ОСНОВНЫЕ МЕТОДЫ (упрощенные) =====

        // A. Регистрация пользователя (УПРОЩЕННАЯ ВЕРСИЯ)
        ...
        // B. Вход в систему (УПРОЩЕННАЯ ВЕРСИЯ)
        ...
        // C. Сохранить строки (простой вариант)
        ...
        // D. Показать мои строки (простой вариант)
        private async Task ShowMyStrings()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    HttpResponseMessage response = await client.PostAsync(endpoint, emptyContent);
        ...
        // E. Сортировка строк (простейшая реализация)
        private async Task SortStrings()
        {
            ...
            // Просто получаем строки и сортируем локально (для простоты)
            try
            {
                var emptyContent = new StringContent("{}", Encoding.UTF8, "application/json");
        ...
        private async Task SearchStrings()
        {
            ...
            // Просто получаем строки и ищем локально (для простоты)
            try
            {
                var emptyContent = new StringContent("{}", Encoding.UTF8, "application/json");
                HttpResponseMessage response = await client.PostAsync("/strings/get-all", emptyContent);
        ...
        // H. Изменить пароль (простой вариант)
