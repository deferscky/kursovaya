...
namespace SimpleStringEditorClient
{
    ...
    class Program
    {
        ...
        private async Task SearchStrings()
        {
            ...
            try
            {
                HttpResponseMessage getResponse = await client.GetAsync("/strings/get-all");
                
                if (!getResponse.IsSuccessStatusCode)
                {
                    string errorText = await getResponse.Content.ReadAsStringAsync();
                    Console.WriteLine($" Ошибка получения строк: {getResponse.StatusCode}");
                    Console.WriteLine($"Ответ: {errorText}");
                    return;
                }
                
                string getJson = await getResponse.Content.ReadAsStringAsync();
                var stringResult = JsonSerializer.Deserialize<StringResponse>(getJson, new JsonSerializerOptions 
                { 
                    PropertyNameCaseInsensitive = true 
                });
                
                if (stringResult?.strings == null || stringResult.strings.Length == 0)
                {
                    Console.WriteLine("Нет сохраненных строк для поиска");
                    return;
                }
                ...
                string searchJson = JsonSerializer.Serialize(searchData);
                var searchContent = new StringContent(searchJson, Encoding.UTF8, "application/json");
                
                HttpResponseMessage response = await client.PostAsync("/strings/search", searchContent);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    try
                    {
                        ...
                        if (root.TryGetProperty("message", out var messageProp))
                        {
                            ...
                            if (root.TryGetProperty("executionTimeMs", out var timeProp))
                            {
                                Console.WriteLine($" Время выполнения: {timeProp.GetInt64()} мс");
                            }
                        }
                        else if (root.TryGetProperty("found_count", out var countProp))
                        {
                            int foundCount = countProp.GetInt32();
                            Console.WriteLine($"\nНайдено {foundCount} совпадений:");
                            
                            if (root.TryGetProperty("results", out var resultsProp))
                            {
                                foreach (var result in resultsProp.EnumerateArray())
                                {
                                    if (result.TryGetProperty("index", out var indexProp) && 
                                        result.TryGetProperty("line", out var lineProp))
                                    {
                                        int index = indexProp.GetInt32();
                                        string line = lineProp.GetString() ?? "";
                            ...
                            if (root.TryGetProperty("executionTimeMs", out var timeProp))
                            {
                                Console.WriteLine($"\nВремя выполнения: {timeProp.GetInt64()} мс");
                            }
                        }
                    }
                    catch (JsonException ex)
                ...
                {
                    string errorText = await response.Content.ReadAsStringAsync();
                    Console.WriteLine($" Ошибка поиска: {response.StatusCode}");
                    Console.WriteLine($"Ответ: {errorText}");
                }
            }
            catch (HttpRequestException ex)
