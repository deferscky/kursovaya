...
// Добавляем CORS для поддержки клиентских запросов
...
// Регистрируем DBManager как сервис с областью действия Singleton (но через DI, не паттерн Singleton)
builder.Services.AddSingleton<DBManager>(serviceProvider =>
{
...
// Функция-хелпер для получения DBManager из DI
...
// Удалить все строки пользователя
